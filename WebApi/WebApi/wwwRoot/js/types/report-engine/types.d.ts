...
export type ReportSection<T> = ReportHeaderSection<T> | ReportTotalSection<T> | ReportDetailSection<T> | ReportGroupSection<T>;
...
export type AggregateFunction = 'sum' | 'avg' | 'min' | 'max' | 'median' | 'values' | 'distinct';
export type SummaryDefinition<T> = {
    [K in keyof T]?: AggregateFunction[];
};
export interface GroupContext<T> {
    all: {
        [key: string]: GroupAccumulatedData<T>;
    };
    recordCount: number;
}
...
export type ReportState<T> = ReportBaseState<T> & Record<GroupKey, GroupContext<T>>;
